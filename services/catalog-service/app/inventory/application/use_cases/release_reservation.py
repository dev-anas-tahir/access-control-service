from app.inventory.application.dto import InventoryResult, ReleaseReservationInput
...
class ReleaseReservationUseCase:
    ...
    async def execute(self, input: ReleaseReservationInput) -> InventoryResult:
        async with self._uow_factory() as uow:
            inv = await uow.inventory.find_by_variant_id(input.variant_id)
            if not inv:
                raise InventoryNotFoundError()

            inv.release(input.quantity)
